...
fn get_geiger_count(path: &Path) -> Result<u64> {
    let mut count = 0;
    for counters in iter_rs_files_in_dir(path).flat_map_ok(|path| {
        cargo_geiger::find_unsafe_in_file(
            &path,
            cargo_geiger::IncludeTests::No,
            cargo::core::shell::Verbosity::Quiet,
            false, /* allow partial results */
        )
    }) {
